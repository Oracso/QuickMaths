...
struct CoreDataPreviewManager {
    ...
    func initExampleData() {
        ...
        createAllExampleObjects([.gameLog, .XXX], 5)
        
        // Add entity relationships

        // linkOneToManyEntities(GameLog.self, XXX.self, .XXX)
    ...
    private func createObjectFromRandomisedObjectData<T: NSManagedObject>(entityType: EntityType, _ ran: RandomisedObjectData) -> T {
        ...
        switch entityType {
        case .gameLog:
            return modifier.createGameLog() as! T
        case .XXX:
            return modifier.createXXX() as! T
        }
    ...
    static func fetchObjectWithBinding<T: NSManagedObject>(_ entityType: EntityType) -> Binding<T> {
        ...
        switch entityType {
        case .gameLog:
            object = fetchPreviewObject(.gameLog)
        case .XXX:
            object = fetchPreviewObject(.XXX)
        }
